...
function YellowCards() {
  ...
  useEffect(() => {
    const cacheKey = `topYellows-${id}-${currentSeason}`;
    ...
    const fetchTopYellows = async () => {
      try {
        const response = await fetch(`https://v3.football.api-sports.io/players/topyellowcards?league=${id}&season=${currentSeason}`, {
          method: 'GET',
          headers: {
