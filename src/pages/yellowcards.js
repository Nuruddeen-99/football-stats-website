...
function YellowCards() {
  ...
  const [topYellows, setTopYellows] = useState([]);
  const [error, setError] = useState(null);
  const [loading, setLoading] = useState(true);
  const navigate = useNavigate();

  useEffect(() => {
    const cacheKey = `topYellows-${id}-${newSeason || season}`;
    const cachedData = sessionStorage.getItem(cacheKey);
  
    if (cachedData) {
      setTopYellows(JSON.parse(cachedData));
      setLoading(false);
      return; 
    }
  
    const fetchTopYellows = async () => {
      try {
        const response = await fetch(`https://v3.football.api-sports.io/players/topyellowcards?league=${id}&season=${newSeason || season}`, {
          method: 'GET',
          headers: {
    ...
    fetchTopYellows(); //eslint-disable-next-line
  }, [id, newSeason, season]);  
  ...
  const handleSeasonChange = (e) => {
    ...
    navigate(`/standings/${id}/${selectedSeason}/yellow-cards`, { replace: true });
      ...
      <div className="container">
        ...
        <div className='gcontent'>
          {loading ? (
            ...
            <div><br />
              <div>
                <img src={topYellows[0]?.statistics[0]?.league?.logo} alt="League" className='limg' />
                <span className='hd'>{topYellows[0]?.statistics[0]?.league?.name}</span>
              </div>
              ...
              <table className='wg_table' style={{ width: "100%", marginTop: "20px" }}>
                <thead>
                  <tr className='wg_header'>
                    ...
                    <th className='wg_text_center' width="20%">Yellow Cards</th>
                  </tr>
                </thead>
                <tbody>
                  {topYellows.map((playerObj, index) => {
                    ...
                    return (
                      <tr key={player.id}>
                        ...
                        <td className='wg_text_center'>{stats.cards.yellow}</td>
                      </tr>
                    );
                  })}
                </tbody>
