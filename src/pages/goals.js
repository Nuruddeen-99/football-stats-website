...
function Goals() {
  ...
  useEffect(() => {
    ...
    if (cachedData) {
      setTopScorers(JSON.parse(cachedData));
      setLoading(false);
      return; 
    }
    ...
    const fetchTopScorers = async () => {
      try {
        ...
        // Save response to state and cache
  ...
  return (
    ...
    {loading ? (
      ...
      <div><br />
        <div>
          <img src={topScorers[0]?.statistics[0]?.league?.logo} alt="League" className='limg' />
          <span className='hd'>{topScorers[0]?.statistics[0]?.league?.name}</span>
        </div>
        <table className='wg_table' style={{ width: "100%", marginTop: "20px" }}>
          <thead>
            <tr className='wg_header'>
              ...
              <th className='wg_text_center' width="20%">Goals (Penalties)</th>
            </tr>
          </thead>
          <tbody>
            {topScorers.map((playerObj, index) => {
              ...
              return (
                <tr key={player.id}>
                  ...
                  <td className='wg_text_center'>
                    {stats.penalty.scored > 0 ? `${stats.goals.total} (${stats.penalty.scored})` : stats.goals.total}
                  </td>
                </tr>
              );
            })}
